...
def entropy(class_y):
	...
	if len(class_y)>0:
		prob_zero=class_y.count(0)/len(class_y)
		prob_one=class_y.count(1)/len(class_y)
		entropy=-(prob_zero*(np.log2(prob_zero))+prob_one*(np.log2(prob_one)))
	else:
		entropy=0
	if np.isnan(entropy):
		entropy=0
...
def information_gain(previous_y, current_y):
	...
	Hl=entropy(l_child)
	Hr=entropy(r_child)
	Pl=len(l_child)/len(parent)
	Pr=len(r_child)/len(parent)
